...
export default function BrochureDownloadPage({ brochure }: BrochureDownloadPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess({ emailSent: Boolean(data.emailSent) })
      setForm(emptyForm())
      window.setTimeout(triggerDownload, 150)
    } catch {
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-sm">
            {!success ? (
              <>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button type="submit" className="btn-primary w-full justify-center" disabled={submitting}>
                    {submitting ? 'Preparing download…' : brochurePage.submitLabel}
              ...
              <div className="text-center">
                ...
                {success.emailSent && (
                  <p className="hp-body text-sm mb-4">{brochurePage.successEmailNote}</p>
                )}
